...
function viewMatch() {
  ...
  const lines = (sideKey) => M.scorerLines(allEvts, m.id, sideKey, playerName)
    .map(l => l.pid
      ? `<button class="gline" data-player="${l.pid}">${l.name ? esc(l.name) + ' ' : ''}${l.mins.map(esc).join(', ')}${l.og ? ' (OG)' : ''}</button>`
      : `<i class="gline">${l.name ? esc(l.name) + ' ' : ''}${l.mins.map(esc).join(', ')}${l.og ? ' (OG)' : ''}</i>`)
    .join('');

  const side = (id, score, other, sideKey) => `
    <div class="sl ${lead(score, other)}" style="--c:${colOf(id)};--tc:${txtOf(id)}">
      ${id ? `<button class="bdg" data-team="${id}"><img src="${crest(id)}" alt=""></button>` : '<span class="bdg"></span>'}
      ${id ? `<button class="who" data-team="${id}"><b>${esc(nameOf(id))}</b><i>${esc(cityOf(id))}</i></button>`
...
function suspensionNotice(m) {
  ...
  const rows = out.map(s => `<li><button class="nmx plink" data-player="${s.id}">${esc(s.player)}</button>
...
function eventText(x, m) {
  ...
  const by = nm ? `<button class="plink" data-player="${x.p}"><b>${esc(nm)}</b></button>` : null;
  switch (x.t) {
...
function viewTables() {
  ...
  const tbl = (g) => M.standings(teamsArr, ms, g, state.ties).map((r, i) => `
      <tr class="${i < 2 && !r.team.disqualified ? 'up' : ''} ${r.team.disqualified ? 'dq' : ''} ${r.unresolved ? 'unres' : ''}">
        <td class="nm"><button class="in" data-team="${r.team.id}"><span class="rk">${i + 1}</span>
          ...
          <i>${esc(r.team.city)}</i></span></button></td>
...
function viewAwards() {
  ...
  const linked = (aw) => aw.winners
    .map((id, i) => `<button class="alink" data-player="${id}">${esc(aw.names[i])}</button>`)
    .join(' &amp; ');
  ...
  const glove = a.goldenGlove.winners.length
    ? card('Golden glove',
        a.goldenGlove.winners.map(w => `<p class="an"><button class="alink" data-team="${w.team.id}">${esc(w.team.city)}</button></p>`).join('') +
  ...
  const table = a.goldenGlove.conceded
    ...
    .map(c => `<tr><td class="nm"><button class="in" data-team="${c.team.id}">
        <span class="tile" style="--c:${c.team.colour}"><img src="${crest(c.team.id)}" alt=""></span>
        <span class="who"><b>${esc(c.team.city)}</b></span></button></td>
